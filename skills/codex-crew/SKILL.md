...
- **Coding-first (standing user preference).** Actual code-writing work prefers **Codex Sol** over Opus-tier claudius agents (`developer-bilby`). This overrides `grand-admiral`'s Token Economy model tiering for implementation tasks.
...
1. **Write scope = cwd + configured `writable_roots`.** On this host `writable_roots` includes `/data/git-worktrees`, `/data/tmp`, `/data/artifacts`, and `network_access = true`. So worktrees under `/data/git-worktrees/<slug>` (the mandatory global worktree location) **are** writable by Codex, scratch under `/data/tmp` and `/data/artifacts` is writable, and sandboxed tests **can** bind localhost sockets. Paths outside cwd and `writable_roots` are read-only.

2. **Codex CANNOT `git commit` in a linked worktree — the coordinator commits on its behalf.** A linked worktree's git metadata lives in the *main* repo's external `.git/worktrees/<name>/` (outside the sandbox), and a separate git-metadata read-only block applies even inside an allowed path. This is normal, not a failure to troubleshoot. **Pattern: Codex writes the files; the coordinator (unsandboxed) runs `git add`/`git commit`.** Plan every Codex dispatch with a coordinator commit step — never wait on Codex to commit.

3. **All worktrees live under `/data/git-worktrees/<slug>`** (global rule; slug = the startup `$PWD` path). The coordinator pre-creates the worktree per `grand-admiral` § Worktree Isolation and injects its absolute path into the dispatch.
