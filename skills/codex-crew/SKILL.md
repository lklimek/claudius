...
- **Default: Codex Astra = `--model gpt-6-astra --effort high`. Always high effort.** State both flags on every dispatch — omitting either drops to the runtime default, not Astra. Astra is gated behind OpenAI's Trusted Access/Daybreak programme (ChatGPT-plan auth may reject it); confirm account access before assuming it resolves.
- **Security-related dispatch: `--model gpt-daybreak-blue-latest --effort high` instead of Astra, when available.** Blue is an alias of `gpt-5.6-sol` (not Astra) with reduced refusals for authorized defensive work — Astra refuses far more cyber requests. Applies to any Codex task that is itself security work — security audits/reviews, auth/crypto/secrets handling, vulnerability triage or remediation, dependency security review — not to ordinary feature/bugfix code that happens to touch an authenticated endpoint. Blue needs separate Trusted Access for Cyber approval (chatgpt.com/cyber), so availability is per-account; treat "when available" literally: if the dispatch's job record shows `status: failed` with an unknown-model or access-gate/auth error, redispatch the same prompt on Astra instead and tell the user the fallback happened. Never silently retry Daybreak Blue more than once per dispatch.
