...
2. **Model tiering (mandatory)**: set model on every spawn — the agent's frontmatter `model:` is only the fallback when you don't. **Sonnet 5** (the `sonnet` alias, which auto-resolves to it) is the capable default workhorse: ~91% of Opus on SWE-bench Pro, best-in-class terminal/computer-use, strong self-verification, native 1M context, ~1.67× cheaper than Opus (2.5× cheaper until 2026-08-31). Tier per agent by where quality is load-bearing:
   ...
   - **Sonnet 5** — agentic-but-routine: the coordinator, `project-reviewer-adams` (review), `technical-writer-trillian` (docs), `Explore` / `general-purpose` (search), and terminal / GUI / browser verification (`playwright-cli`, `desktop-gui`, `verify` — Sonnet 5 leads OSWorld / Terminal-bench).
   - **Haiku** — trivial mechanical (bulk search, formatting).
   Override per task, both ways: downgrade a quality-critical agent to Sonnet 5 for a trivial job; upgrade a routine agent to Opus for a genuinely hard one. **Risk-based tiebreaker**: a security-sensitive review — crypto, auth/key handling, network/transport, deserialization, untrusted input, or a large/opaque diff — is Opus even when generic review is Sonnet 5; routine low-risk security (clean version bumps with passing vuln scan, mechanical refactors) stays Sonnet 5/Haiku. When unsure: tier up for security, down for cost. **Tokenizer caveat**: Sonnet 5 emits 1.0–1.35× more tokens than Sonnet 4.6 — still net cheaper, but watch cache-heavy sessions.
