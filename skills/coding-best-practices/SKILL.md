...
- **Describe present state, not history**: comments document what code does NOW and why. Historical context — refactors, prior approaches, renames, evolution, "previously did X", "TODO: clean up old approach" — belongs in commit messages and PR descriptions. The reader has `git blame`. Acceptable exceptions are rare: citing an external constraint (upstream issue, RFC, kernel API quirk) that justifies a non-obvious current choice. Composes with `rust-best-practices` M-NO-TOMBSTONES — same principle, different framing.
