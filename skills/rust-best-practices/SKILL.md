...
- **Never override `CARGO_TARGET_DIR`/`--target-dir`, except the documented concurrent-worktree isolation case** — the machine's `~/.cargo/config.toml` already routes every build (worktrees included) to its configured shared target-dir and sccache. The hook denies ad-hoc overrides; see `grand-admiral` § Worktree Isolation for the one documented exception (per-agent target dir + `CLAUDIUS_ISOLATE_TARGET=1`, concurrent same-project worktree waves only).
