...
- **Fixed — non-finite floats could reach disk as invalid JSON**: `reject_non_finite_constant` only ever saw bare `NaN`/`Infinity` tokens, so an overflowing literal like `1e400` — an ordinary JSON number Python silently decodes to `inf` — passed every loader, survived each `isinstance`/range check, and was re-serialized by the writer as a bare `Infinity` token, producing a report file no conforming JSON reader can load. Added `reject_non_finite_number` (a `parse_float` guard) and `load_json_strict`, which applies both guards as one call so a new loader cannot pick up one and miss the other; every report loader now goes through it, including `merge_findings_helper.py`, which had no guard at all and sits mid-pipeline between `prepare` and `assemble`. Both JSON writers gained `allow_nan=False` as a backstop.
- **Fixed — `regenerate` wrote v4 floats under a v3 declaration**: `consolidate_reports.py regenerate` loads through the float migration, so what it writes back is a v4 document, but it left the declared `schema_version` at its legacy value — a report contradicting its own field names, which `validate_report.py --strict-v4` would then certify. It now restamps to the emitted version and logs the change.
- **Fixed — "enforce" overstated the gate checks**: the gate-citation checks are advisory `[consistency]` warnings that never change `validate_report.py`'s exit code; docstrings and changelog wording now say so instead of implying a hard gate.
