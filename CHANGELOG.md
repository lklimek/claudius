...
- `git-and-github` Creating a PR + `coding-best-practices` present-state rule: PR descriptions now explicitly describe net final state only — no history, changelog, or iteration narrative. `coding-best-practices` reconciled to match: historical context belongs in commit messages only (removed "PR descriptions" as a sanctioned destination for history).
