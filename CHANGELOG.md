...
- `scripts/agent-watchdog.sh` — a persistent agent-stall watchdog for the Monitor tool, filling the one gap the harness does not auto-notify on: an agent that owns assigned work yet has gone silent. A stall is defined as **owning an in_progress task AND idle past the threshold AND no build running**, NOT bare idle — a healthy agent idles while waiting for its next instruction, and an idle agent with no in_progress task is never flagged. "Owns work" is read each poll from the on-disk task store (`~/.claude/tasks/<teamName>/<id>.json`, derived from the team config `name`; `owner`+`status` fields are the source of truth). Discovers the UNION of three sources (dedup by label, team wins): **team** members (newest `~/.claude/teams/session-*/config.json`, `isActive==true`, `agentType != "team-lead"`) — NAMED, task-gated, activity = newest mtime under the member worktree else `cwd`; **worktree-isolated** agents (`.claude/worktrees/agent-*`) — NAMED, task-gated, activity = newest mtime under the dir; and **individual/background subagents** (`…/<leadSessionId>/subagents/agent-*.jsonl`) — ANONYMOUS, NOT gated (run one task to completion, don't idle-wait; the harness notifies on completion), activity = transcript mtime scoped to the current lead session so stale prior-session transcripts are never flagged. STALL is suppressed while any build runs (cargo/rustc/go/node/pytest/jest/tsc/webpack/npm/pnpm/yarn). Missing-signal agents are SKIPPED, never defaulted to the Unix epoch (which would emit bogus ~56-year idle alerts). **Silent when healthy**: strictly edge-triggered, emitting only `STALL`/`RESUMED` transition lines to stdout (diagnostics to stderr), so it costs zero coordinator tokens until an agent actually stalls. Verified against the live session (member `bilby-2` owns an in_progress task but is actively working → zero output; an idle agent owning no in_progress task → zero output).

### Changed

- `grand-admiral` `## Recovery` doctrine fully rewritten. Harness death/completion remains the PRIMARY driver; the watchdog covers silently-stuck agents and explicitly spans all THREE agent kinds (team / worktree — both NAMED and task-gated; subagents — ANONYMOUS, ungated), framing a stall as **owning an in_progress task + idle + no build** and calling out that an idle agent with no assigned in_progress task is healthy and never flagged. Monitor one-liner uses the portable plugin-root path `${CLAUDE_SKILL_DIR}/../../scripts/agent-watchdog.sh` (resolves to the install location at skill-load time; the relative path would break since the Monitor's CWD is the user's repo). The watchdog's **silent-when-healthy / zero-token** property is stated. STALL event format documented (`task-owned=in_progress` for named, `reason=subagent-idle` for subagents). STALL-handling procedure (investigate-before-acting): inspect transcript + `git -C <cwd>` state, re-nudge a live-but-idle owner via `SendMessage`, or archive the inbox to `inboxes/<name>.json.killed-<ts>` and respawn the same agent-type on the SAME cwd/worktree with a context brief (transcript tail, `git log`, branch) and the SAME tasks re-fed via `TaskGet`, so committed progress survives. Escalate to user only after a second attempt fails. The allow-rule (`Bash(*agent-watchdog.sh *)`) lives in settings, not skill frontmatter — an `allowed-tools` allowlist on this always-loaded coordinator skill would strip its other tools.
