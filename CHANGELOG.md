...
- `scripts/agent-watchdog.sh` — a persistent agent-stall watchdog for the Monitor tool, filling the one gap the harness does not auto-notify on: an agent that is alive but went silent. Discovers the UNION of two sources: team agents (newest `~/.claude/teams/session-*/config.json` members with `isActive==true` — which cleanly excludes both the coordinator, that has no `isActive` field, and finished members) keyed by `name`, activity = newest mtime under the member `cwd` + its `inboxes/<name>.json`; and individual/background subagents (`…/<lead-uuid>/subagents/agent-*.jsonl`) keyed by `agent-<id>`, activity = the transcript's own mtime, scoped to the newest subagents dir to drop completed prior-session agents. STALL fires only when idle past `--stall-secs` AND no build (cargo/rustc/go/node/pytest/jest/tsc/webpack/npm/pnpm/yarn) is running — a healthy agent blocked on one long cold build writes nothing, so build activity conservatively suppresses STALL for all agents. Agents with no files yet are SKIPPED, never defaulted to the Unix epoch (which would emit bogus ~56-year idle alerts). **Silent when healthy**: strictly edge-triggered, emitting only `STALL`/`RESUMED` transition lines to stdout (diagnostics to stderr), so it costs zero coordinator tokens until an agent actually stalls.

### Changed

- `grand-admiral` `## Recovery` doctrine fully rewritten. Harness death/completion remains the PRIMARY driver; the watchdog covers silently-stuck agents and explicitly spans BOTH team agents and individual/background subagents. Monitor one-liner uses the portable plugin-root path `${CLAUDE_SKILL_DIR}/../../scripts/agent-watchdog.sh` (resolves to the install location at skill-load time; the relative path would break since the Monitor's CWD is the user's repo). The watchdog's **silent-when-healthy / zero-token** property is stated. STALL event format documented. STALL-handling procedure (investigate-before-acting): inspect transcript tail + `git -C <cwd>` state, nudge live-but-idle agents via `SendMessage`, or spawn a replacement on the SAME cwd/worktree with a context brief (transcript tail, `git log`, branch, open tasks) and archive the inbox to `inboxes/<name>.json.killed-<ts>` so committed progress survives. Escalate to user only after a second attempt fails. The allow-rule (`Bash(*agent-watchdog.sh *)`) lives in settings, not skill frontmatter — an `allowed-tools` allowlist on this always-loaded coordinator skill would strip its other tools.
