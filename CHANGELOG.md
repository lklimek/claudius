...
- **`skills/grand-admiral/SKILL.md`**: documents the same-HEAD shared-target-dir hazard (cargo's relative dep-info paths let one worktree's binary satisfy another's freshness check, corrupting test verdicts silently — not mere lock contention), the mandatory-upfront per-agent `CARGO_TARGET_DIR`+`CLAUDIUS_FORCE=1` isolation for same-HEAD multi-agent waves plus the required provenance check (grep the ledger log for the specific test names touched, confirm `passed + filtered == expected total` — an aggregate pass count alone isn't proof), the `shutdown_approved`-doesn't-free-the-tmux-pane recovery procedure, that `shutdown_request` cannot preempt a teammate mid tool-call, and a `[COORDINATOR CORRECTION from <name>]` tagging convention so mid-task `SendMessage` steers aren't mistaken for prompt injection.
- **`CLAUDE.md`**: new scripts default to Python over Bash (error handling, testability, no GNU/BSD portability split to work around).

### Changed

- **`skills/ci-dance/SKILL.md`**: replaced the `TeamCreate`/`TeamDelete` + `TaskCreate`/`TaskList`/`TaskUpdate` stream-coordination choreography with the implicit-team spawn model and `SendMessage`-based claim/completion broadcasts — confirmed via this session's own tool inventory that none of those six tools are currently available. `TeamCreate`/`TeamDelete` were intentionally removed (anthropics/claude-code#68721, v2.1.178); the task-board tools have a spottier history (anthropics/claude-code#23816, closed, v2.1.34, reports them missing, while #68721 itself reports them working on 2.1.178) — treat their presence as build-dependent and reverify rather than assume either way. Every session already has one implicit team, so named `Agent()` spawns join it automatically with no create/destroy step.
- **`skills/ci-dance/SKILL.md`**: claim conflicts between streams no longer silently drop a finding — a stream that finds its target already claimed now defers it instead of skipping, and Step 3 (Merge) verifies every deferred finding was actually fixed by its claimant, reassigning or carrying forward anything that wasn't. A finding is never dropped just because another stream claimed it first and then didn't deliver.
- **`skills/grand-admiral/SKILL.md`**, **`skills/review-pr/SKILL.md`**, **`skills/workflow-feature/SKILL.md`**, **`skills/grumpy-review/SKILL.md`**: removed remaining `TeamCreate`/`TeamDelete`/`TaskCreate`/`TaskList`/`TaskUpdate`/`TaskGet` prescriptions (Session Protocol, Spawning, Recovery, frontmatter `allowed-tools`) missed by the `ci-dance` rewrite above — coordinators now track progress via a plain checklist and coordinate multi-agent work via named spawns + `SendMessage`, matching the model `ci-dance` already used.
- **`skills/coding-best-practices/SKILL.md`**, **`skills/grand-admiral/SKILL.md`**: reworked the verification-timing rule — targeted scope now applies at every stage, including the merge gate and branch-merge points; there is no mandatory full-suite local run, since CI is the full-suite backstop. Supersedes 5.5.0's original scoped-testing/full-suite-at-gate rule, revised within the same unreleased version after feedback that a local full run duplicates what CI already covers. The isolation-for-full-suite-evidence bullet is removed as moot — with no mandated local full-suite run left to trust, there's nothing to isolate. The separate, bug-motivated per-agent `CARGO_TARGET_DIR` mandate for concurrent same-project worktree waves (Worktree Isolation § Same-HEAD hazard) is unrelated and unchanged: it isn't about evidence sufficiency, it prevents a confirmed cross-agent binary collision regardless of test scope.
...
- **`skills/grand-admiral/SKILL.md`**: corrected the `CLAUDIUS_FORCE=1` description — it's a global allow evaluated before every cargo-discipline hook rule, clearing all four (including the ledger-routing requirement), not just the target-dir-override denial as previously stated; the mandated per-agent isolation command still routes through `cargo-cached.sh` deliberately even though the hook can no longer enforce that once the token is present. Also reworked the `[COORDINATOR CORRECTION from <name>]` convention: the tag alone is a static, forgeable string, so a receiving agent should only act on a correction that also references specifics unique to its own assignment, not the bare tag. Found via independent security review of this same PR's earlier commits.
- **`hooks/session-start.sh`**, **`skills/rust-best-practices/SKILL.md`**: carved out the documented concurrent-worktree `CARGO_TARGET_DIR` isolation exception in the "never override" guidance — both previously stated the rule unqualified, directly contradicting `grand-admiral`'s mandate for concurrent same-project worktree waves and risking that the mandated mitigation never actually gets followed. Found via independent security review.
- **`skills/ci-dance/SKILL.md`**: noted explicitly that stream claims are self-asserted, unauthenticated text — the Review Stream in particular ingests externally-sourced GitHub PR comments — so Step 3's verification, not the claim, is the real trust boundary. Reinforces the defer-and-verify fix above rather than changing its behavior. Found via independent security review.
