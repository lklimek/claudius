...
- **`scripts/agent-watchdog.py`** `CodexScanner.scan()`: warn once when a discovered job's self-reported `workspaceRoot` doesn't canonical-match the candidate being scanned, instead of silently discarding it with no trace — every other skip path in this function already warned; this was the one exception, and it's exactly the failure mode that let a real Codex job go permanently unreported in a separate incident. Matching behavior itself is unchanged.
...
- **`skills/codex-crew/SKILL.md`** + `references/sandbox-and-recovery.md`: Codex can now `git commit` directly inside a linked worktree (verified empirically 2026-07-16, commit `f2639aa`) — reverses the earlier hard-block guidance. `writable_roots` was unchanged at test time; the likely enabling lever is `approval_policy = "on-request"` + the project's `trust_level = "trusted"`, not a sandbox-path change. **Confirmed inconsistent the same day**: a later dispatch in a different worktree hit the exact old `index.lock`/read-only error (commit `963db57`/`7c2d3e8` committed by the coordinator instead) — coordinator-commit is the reliable default, not just a fallback for a regression.
