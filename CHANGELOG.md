...
- `scripts/agent-watchdog.sh` — a persistent agent-stall watchdog for the Monitor tool, filling the one gap the harness does not auto-notify on: an agent that is alive but went silent **with work still pending**. A stall is defined as pending-work + idle, NOT bare idle. Discovers the UNION of three sources, dedup by label (team wins): **team** agents (newest `~/.claude/teams/session-*/config.json` members with `isActive==true` and `agentType != "team-lead"`, which excludes both the coordinator and finished members) — judged by their inbox `inboxes/<name>.json`, where an empty inbox (`[]`, ≤2 bytes) is healthy and NEVER fires and only an inbox holding unprocessed messages past the threshold stalls (`reason=inbox-unprocessed`); **individual/background subagents** (`…/<leadSessionId>/subagents/agent-*.jsonl`) — judged by transcript mtime, scoped to the current lead session (via the team's `leadSessionId`) so stale prior-session transcripts are never flagged (`reason=subagent-idle`); and **worktree-isolated** agents (`.claude/worktrees/agent-*`) — judged by newest mtime under the dir (`reason=worktree-idle`). STALL is suppressed while any build runs (cargo/rustc/go/node/pytest/jest/tsc/webpack/npm/pnpm/yarn — a healthy agent blocked on a long cold build writes nothing). Missing-signal agents are SKIPPED, never defaulted to the Unix epoch (which would emit bogus ~56-year idle alerts). **Silent when healthy**: strictly edge-triggered, emitting only `STALL`/`RESUMED` transition lines to stdout (diagnostics to stderr), so it costs zero coordinator tokens until an agent actually stalls. Verified against the live harness state (an idle team member with an empty inbox produces zero output).

### Changed

- `grand-admiral` `## Recovery` doctrine fully rewritten. Harness death/completion remains the PRIMARY driver; the watchdog covers silently-stuck agents and explicitly spans all THREE agent kinds (team / individual-background / worktree), framing a stall as pending-work + idle and calling out the empty-inbox-is-healthy property. Monitor one-liner uses the portable plugin-root path `${CLAUDE_SKILL_DIR}/../../scripts/agent-watchdog.sh` (resolves to the install location at skill-load time; the relative path would break since the Monitor's CWD is the user's repo). The watchdog's **silent-when-healthy / zero-token** property is stated. STALL event format documented (`reason=inbox-unprocessed|subagent-idle|worktree-idle`). STALL-handling procedure (investigate-before-acting): inspect transcript + `git -C <cwd>` state, re-nudge a live-but-pending agent via `SendMessage`, or archive the inbox to `inboxes/<name>.json.killed-<ts>` and respawn the same agent-type on the SAME cwd/worktree with a context brief (transcript tail, `git log`, branch) and the SAME tasks re-fed via `TaskGet`, so committed progress survives. Escalate to user only after a second attempt fails. The allow-rule (`Bash(*agent-watchdog.sh *)`) lives in settings, not skill frontmatter — an `allowed-tools` allowlist on this always-loaded coordinator skill would strip its other tools.
