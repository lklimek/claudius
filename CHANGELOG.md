...
- **`grand-admiral` § Session Protocol**: search persistent memory on **surprise** — an unexpected result, a repeating error, an approach failing for a non-obvious reason — not only at session start, when what will confuse the session is not yet known. Query the literal tokens on screen (command as typed, error text verbatim) kept short, since a full-sentence symptom description retrieves measurably worse; and treat one miss as a bad query rather than proof nothing is stored, because a record can be invisible to every symptom phrasing yet top-ranked for a mechanism keyword.
## [7.0.1] - 2026-07-31

### Fixed

- **non-finite floats could reach disk as invalid JSON**: `reject_non_finite_constant` only ever saw bare `NaN`/`Infinity` tokens, so an overflowing literal like `1e400` — an ordinary JSON number Python silently decodes to `inf` — passed every loader, survived each `isinstance`/range check, and was re-serialized by the writer as a bare `Infinity` token, producing a report file no conforming JSON reader can load. Added `reject_non_finite_number` (a `parse_float` guard) and `load_json_strict`, which applies both guards as one call so a new loader cannot pick up one and miss the other; every report loader now goes through it, including `merge_findings_helper.py`, which had no guard at all and sits mid-pipeline between `prepare` and `assemble`. Both JSON writers gained `allow_nan=False` as a backstop.
- **`regenerate` wrote v4 floats under a v3 declaration**: `consolidate_reports.py regenerate` loads through the float migration, so what it writes back is a v4 document, but it left the declared `schema_version` at its legacy value — a report contradicting its own field names, which `validate_report.py --strict-v4` would then certify. It now restamps to the emitted version and logs the change.
- **"enforce" overstated the gate checks**: the gate-citation checks are advisory `[consistency]` warnings that never change `validate_report.py`'s exit code; docstrings and changelog wording now say so instead of implying a hard gate.
- **the recipe guard skipped the doctrine's own examples**: `tests/test_documented_severity_recipes.py` verified the producer skills' documented floats but not the two worked examples inside `severity/SKILL.md` itself, so the document defining the derivation was the one document free to contradict it. Both are covered now, and their float spellings were normalized to the canonical `likelihood = X, impact ≈ Y` form the guard parses.
- **Report counts depended on load order**: `_normalize_report` decided whether to force a summary-statistics rebuild from whether a migration had just run, so a v3-declaring report whose floats were already in v4 form kept stale counts. It now also checks the declared `schema_version`, making the decision order-independent.
- **Consolidation could fail quiet instead of loud**: a finding whose severity floats are unusable now takes the highest surviving dimension (`effective_severity`) rather than sinking to INFO, and non-finite values no longer pass as numbers. For a review tool the damaging failure is under-reporting, so every default errs upward.

## [7.0.0] - 2026-07-31

### Changed

- **BREAKING — `severity` skill's blocking doctrine replaced**: the old "introduced by this diff AND material" blocking test is gone, replaced by per-finding **backstop zones** (Backstopped/Sovereign/Boundary, judged from PR context, never a standing repo profile) that cap `impact`, plus 16 named **blocker gates** (`G-INTENT`, `G-DATA`, `G-FUNDS`, `G-CRYPTO`, `G-SECRET`, `G-UI-BROKEN`, `G-UI-TEXT`, `G-WYSIWYS`, `G-AMOUNT`, `G-DOUBLE`, `G-PHISH`, `G-BRICK`, `G-SILENT`, `G-PRIVACY`, `G-GROWTH`, `G-DEFAULTS`) that decide `merge_class: blocking`. A `blocking` finding's `intent_basis` must now name the tripped gate ID plus one line of evidence (e.g. `"G-SECRET: seed phrase written to debug log at wallet/import.rs:88"`) — a bare requirement quote no longer qualifies. Pre-existing findings tripping `G-FUNDS`/`G-SECRET`/`G-CRYPTO`/`G-DATA` must be escalated to the human explicitly, never silently deferred. `validate_report.py` checks the citation in both directions (`blocking` without a valid `G-*:` citation; a valid citation on any other class) — as **advisory `[consistency]` warnings that never change the exit code**, not a hard gate.
- **BREAKING — severity float fields renamed and de-OWASP'd**: schema/report field `risk` → `likelihood`; `scope` → `relevance` (blast radius folds into `impact` instead). `overall_severity` is now `(likelihood + impact) / 2` — `relevance` is excluded from the severity mean entirely (previously the 3-term average laundered a pre-existing catastrophe down to MEDIUM); `relevance` now drives only `merge_class` and report ordering. Schema bumped `3.2.0` → `4.0.0`; `3.x` is accepted read-only for in-flight reports and migrated to the v4 field names on load — a migrated finding with no `relevance` gets it defaulted to `0.5` and **MUST be re-rated**, since an unrated default can silently misclassify `merge_class` on a real defect (see `scripts/severity_util.py`'s `migrate_legacy_floats`). All producer skills (`review-pr`, `grumpy-review`, `check-pr-comments`, `review-dependency`), `report-format`, `validate-findings`, and `triage-findings` updated to the new field names, the 2-term derivation, gate-cited `intent_basis`, and the `4.0.0` schema version. `severity`'s external-reviewer compatibility map documents the v3 mapping for consumers of older reports: `risk` → `likelihood` by rename, while v3 `scope` maps to **nothing** — blast radius folds into `impact`, and carrying a `scope` value into `relevance` would misclassify `merge_class`, so `migrate_legacy_floats` discards it and requires re-rating.
- **Reports de-inflate by roughly one severity band.** Producer float recipes were written when `scope` defaulted to `1.0`, which pulled every 3-term mean upward; the 2-term mean removes that lift, so a canonical `0.6`/`0.7` finding derives MEDIUM where it previously derived HIGH. This is deliberate — it is the correction the old skill asked for when it warned that a lazy `scope=1.0` "floors the mean at MEDIUM and inflates every report" — and the recipe numbers were left as authored rather than re-tuned to preserve their former labels, which would back-fit floats to a desired band. Affects `top_findings` selection, report ordering, and remediation buckets; it does **not** affect `merge_class` (gates decide blocking) or inline-comment eligibility (CRITICAL–LOW).
- **Absorbed from 6.5.0's context-aware scoring**: the non-adversarial scoring guidance (execution frequency, precondition probability, triggering actor) and its 🔴 evidence rule now sit on `likelihood` — a low operational score must cite a call-tree trace, an evidenced Context Digest claim, or a human statement, and **unknown is not benign**: with no evidence, score generically. The Context Digest remains a first-class scoring input in `grumpy-review` and `review-pr`. Superseded by this release: 6.5.0's `scope`-as-blast-radius-only definition, and both halves of its softening of `out_of_scope_follow_up` — the prose *and* the decision-tree branch routing "fixing it grows the PR beyond its stated intent" to that class, which the base merge spliced in silently and which contradicted the retained doctrine twenty lines below it — blast radius folds into `impact` here, and the class keeps its "acceptable to never fix" reading, with deferrals surfaced to the user at presentation time rather than silently dropped.
- **`validate_report.py` gained `--strict-v4`**: schema-v3 input is migrated in memory and validated in that form, and the verdict now says so rather than certifying an on-disk file it never validated; `--strict-v4` rejects v3 outright for callers needing v4 on disk.
- **Producer mode now runs the merge-classification checks.** `--producer` previously skipped `check_consistency` wholesale, which is exactly how the three producers permitted to emit `merge_class` inline were validated — so every inline-authored gate citation went unchecked. Behavior change for existing `--producer` callers: they can now emit `[consistency]` warnings (advisory, still exit 0).
- **Fixed — schema rejected a field the docs mandate**: `grumpy-review` §3 instructs every reviewer to emit `cross_domain_hint`, which `additionalProperties: false` rejected. Added to the finding schema.
...
- **Context-aware review doctrine** — reviewers and fixers now get the code's operational reality instead of guessing at it:
...
- **`severity`**: `scope` is de-conflated to blast radius only — PR-relevance and intent-matching live exclusively in `merge_class` (the HTML `scope` chip tooltip claimed "PR relevance" too — corrected). The decision tree's "must not survive this review" branch now splits: beyond the PR's stated intent → `out_of_scope_follow_up`, otherwise `non_blocking`. An `out_of_scope_follow_up` finding is **surfaced, then let go**: a deferral that never gets actioned is usually the correct outcome (the finding wasn't worth prioritizing), so the class needs no tracking field, filing step, or gate — only visibility. No review skill files a GitHub issue or any other tracking item, since choosing between `claudius:triage-findings`, `memcan:todo`, and doing nothing is the user's call. Visibility is a coordinator post-processing step over data the report already carries — filter the consolidated findings by `merge_class` and name the deferral candidates when reporting results (`grumpy-review`, `review-pr`, and `ci-dance`'s Final Report each say so) — not a new schema field or render path. HIGH/CRITICAL security findings are never silently deferred — they go to the user as an explicit disposition question.
- **`ci-dance`**: the Grumpy and Review streams route findings by `merge_class`, not raw severity — `blocking`/`non_blocking` get fixed, `out_of_scope_follow_up` is surfaced in the Final Report and never fixed inline, `disputed` is skipped. Exit criterion becomes "no `blocking`, every `non_blocking` fixed or explicitly carried" instead of "no MEDIUM+", so a valid pre-existing MEDIUM this PR neither introduced nor relies on stops being auto-absorbed into it. Claim-deferral is renamed throughout to keep it distinct from merge-class deferral.
...
- **`review-dependency`**: new step 2d cross-checks a dependency update's changelog/release notes against its actual commit history and diff — tag/commit integrity, undocumented files or commits, new/modified install-lifecycle hooks (npm pre/postinstall, Python `setup.py` hooks, CI/release workflow files), added network calls or credential reads, obfuscated source additions, diff-shape mismatches, and contributor provenance. Findings feed the security audit (step 3) and get a dedicated "Diff Integrity" report section; an unresolved finding there floors the overall risk rating regardless of the code-level audit's result.

### Fixed

- **`grumpy-review`**: CI-safe `BASE_BRANCH` resolution (falls back to `origin/$BASE_BRANCH` when the bare ref doesn't resolve locally); collision-resistant scratch directories with a check-before-overwrite convention for concurrent reviews of the same PR; producer spawn prompts now forbid metadata-wrapper envelopes and self-invoking `consolidate_reports.py`; `§5b`'s merge-decision pattern for reviews above ~30 raw findings now runs against a real, tested `scripts/merge_findings_helper.py` instead of an inline SKILL.md snippet; `§5c`'s `assemble` invocation uses `$SCRATCH_DIR` consistently instead of a stray `${TMPDIR:-/tmp}`; spawn prompts list the full deployed roster and support passive cross-domain finding hints; a `TaskStop` cleanup pass after consolidation works around leaked tmux-backend teammate processes.
- **Report pipeline** (`consolidate_reports.py`, `severity_util.py`, `generate_review_report.py`): `prepare`'s computed `agent_stats` is now actually reachable by `assemble` — `§5b` step 7 has the coordinator copy it verbatim from `intermediate.json` into `merged-findings.json` (via the new merge helper script) instead of hand-authoring it, and `compute_statistics` skips non-dict entries defensively instead of crashing with `AttributeError` on a malformed one; `derive_severity_int` gained an epsilon tolerance so exact-boundary severity trios no longer silently drop a full band; the HTML/triage markdown renderer now guards its `import markdown` the same way the PDF renderer already does; `redundancy_ratio` now ships a per-agent breakdown alongside the top-line ratio so resolved-comment collapse isn't misread as reviewer-overlap waste.
- **`codex-crew`**: documents cancelling a harness-killed job (`codex-companion.mjs cancel <job-id> --json`, run from the job's own `--cwd`) before `--resume-last`, since a stale broker-level lock otherwise blocks the resume; promotes a `ps -p <pid>` liveness check into the primary "Monitoring a Codex Job" workflow; corrects the overstated claim that `/data/artifacts` is Codex-writable; every dispatch prompt now carries a standing "you are a leaf worker, not a coordinator" line so Codex stops reading orchestration-only skills it'll never use; stops hardcoding this host's `/data/tmp`/`/data/target`/`/data/artifacts` paths in prose (`SKILL.md` and `references/sandbox-and-recovery.md`), keeping only the literal `~/.codex/config.toml` example values as host-specific.
- **`check-pr-comments` / `git-and-github`**: `gh-fetch-reviews.sh` now actually produces output — its `gh api --paginate --slurp --jq '...'` combined two mutually-exclusive flags and exited 1 on every invocation with zero test coverage to catch it; now a two-process `gh api --paginate --slurp | jq '...'` pipeline, with new regression tests; documents the `ghsudo` requirement for `gh-resolve-review-threads.sh` under the triage-role token; `gh-common.sh`'s stdin buffering now uses a `[ ! -t 0 ]` tty check instead of a racy 0.1s read probe that dropped a genuine-but-slow producer's stdin on a `ghsudo` retry (indistinguishable from no stdin at all under the old probe); documents `add_comment_to_pending_review`'s required camelCase `subjectType` + uppercase `LINE`/`FILE` casing (in `pr-review.md`, no longer duplicated in `SKILL.md`); `check-pr-comments`'s `gh-cli-fallback.md` now delegates to `git-and-github`'s copy instead of duplicating it; documents that a bare coordinator session typically lacks GitHub MCP tools and should default to the `gh` CLI fallback.
- **`grand-admiral`**: clarifies that MemCan tool availability isn't guaranteed for `general-purpose`-type agents despite a `Tools: *` listing; adds a "verify, don't implement on faith" note to Development-Work Delegation; documents native Claude Code task/teammate notifications as the sanctioned watchdog fallback for MCP-tool-less subagents (e.g. `qa-engineer-marvin`); adds a task-notification sanity-check note against cross-session leakage; corrects example agent-brief phrasing that told non-Task-tool agents to invoke `plugin-dev:skill-reviewer` directly; Skills Reference catalog now lists all 29 loadable skills (was missing `bye`, `ci-dance`, `codex-crew`, `delegate`, `push`, `release`, `report-format`, `track-minions`, `validate-findings`); the 7 core agents (`architect-nagatha`, `developer-bilby`, `project-reviewer-adams`, `qa-engineer-marvin`, `security-engineer-smythe`, `technical-writer-trillian`, `ux-designer-diziet`) are granted `mcp__agent-watchdog__register_session` so they can self-register as MCP watchdog children instead of always falling back to doc-only coverage.
- **`hooks/cargo-discipline.sh`**: no longer false-positives on commit-message prose containing literal `cargo test`/`cargo clippy`/`cargo nextest` inside a quoted heredoc body; the heredoc-quote detection now also recognizes a backslash-quoted delimiter (`<<\EOF`), which the initial fix missed, reintroducing the same false-positive class for that quoting style.
...
- **Built-in minion monitor**: renamed the repository's polling script to `scripts/minion-monitoring.py`, including its tests, CLI identity, launch commands, and documentation, to avoid a naming collision with the unrelated external `mcp__agent-watchdog__*` tool.
- **`codex-crew` § Plan-Approval Gate**: retire the broken read-only plan → `--resume-last --write` sequence. Codex app-server write mode is pinned at thread creation and cannot be escalated on resume, as two independent reproductions confirmed. Well-scoped writable work now starts with `--write`; a genuine read-only approval pass hands the approved plan to a fresh writable job. Closes `memcan:todo` `370fff50` and `f9cd373a`.
- **`skills/ci-dance/SKILL.md`**: stream-agent spawn-prompt guidance now forbids ending a stream's turn to wait on a Monitor/background-task notification from a sub-job it spawned (e.g. a Codex dispatch) — that notification can never be delivered back once the turn ends, silently stalling the stream forever; poll the sub-job's status directly instead (see `codex-crew` § Monitoring a Codex Job). A companion coordinator-side note treats a stream's task-notification with no actual report/findings as a possible STALL to investigate, never an assumed clean "nothing found." Closes `memcan:todo` `b61f6f0f`.

### Fixed

- **`scripts/minion-monitoring.py` CLI**: change `--worktrees` precedence to flag > `$CLAUDIUS_WORKTREE_ROOT` > `/data/git-worktrees`, and align every live worktree example with the host doctrine so direct Codex jobs are discovered without an operator override. Closes `memcan:todo` `5f73d6b0`.
- **`scripts/cargo-cached.sh`**: target-dir auto-isolation failures (cargo metadata resolution, missing `timeout`, or a failing `mkdir -p`) now abort loudly (`exit 1`) instead of silently falling through to cargo's shared/default target dir — a shared target dir under concurrent same-commit checkouts could previously report a stale binary as a clean build. Adds a retry (3s then 15s) for transient contention and logs which target dir was actually used (isolated vs. a deliberate caller override) to the ledger log for post-hoc provenance. Closes `memcan:todo` `6afae831`.
...
- **`scripts/agent-watchdog.py`**: `resolve()` now catches `ValueError` alongside `OSError` (a NUL byte in a job's `workspaceRoot` no longer blinds an entire poll); `safe_json` bounds read size and catches `RecursionError`; Codex job-scan failures are now isolated per-job so one malformed `jobs/*.json` file only skips that job instead of discarding the whole poll's Claude+Codex events; `CodexScanner.scan()` bounds jobs enumeration further for long sessions; Source C's worktree glob under the configured root no longer requires a literal `agent-*` prefix (was incompatible with this repo's own `<repo-path-slug>` worktree-naming convention); `member_activity()` now shares that same relaxed match (previously it only checked the exact `agent-{key}` name, silently falling back to the lead's cwd for a correctly-discovered-by-Source-C worktree); independent Codex-job discovery no longer requires team/worktree workspace mapping to already know about a workspace; top-level crash diagnostics added for unreproducible exits (e.g. exit-144); the main-loop poll-failure diagnostic is deduplicated (`warn_once`, keyed by exception type) and its exception text passes through the same sanitizer as Codex job fields, instead of printing raw and unbounded on every poll.
- **`scripts/agent-watchdog.py` CLI**: worktree-root precedence is `--worktrees` > `$CLAUDIUS_WORKTREE_ROOT` > `/data/git-worktrees`.
- **`scripts/agent-watchdog.py` CLI**: zero arguments print usage plus an actionable diagnostic and exit 1; `--help` remains exit 0.
- **`scripts/cargo-cached.sh`**: a `--target-dir`/`--target-dir=<val>` CLI flag is now treated the same as an explicit `CARGO_TARGET_DIR` — captured before auto-derivation, kept in the ledger cache key so two different target dirs no longer collide on one key (previously stripped, which could report a false pass for the wrong target dir), and recorded truthfully in the ledger's `target_dir` field; the ledger replay key now includes checkout-root identity, fixing cross-checkout replay collisions at the same commit; the ledger falls back to a workspace-local `.claudius-ledger` directory when the default `~/.cache/claudius/ledger` root isn't writable (e.g. inside the Codex sandbox), and now prints a loud diagnostic before falling back further to plain unsandboxed `cargo` when both ledger roots are unwritable (previously silent).
- **`scripts/consolidate_reports.py`** `prepare`: a flat array of bare finding objects (no `findings` sub-array) is now detected and auto-wrapped into a synthetic `finding_section` with a WARN log, instead of silently dropping every finding; the same rescue now also applies to bare findings mixed alongside proper sections in one array (previously only an *all-bare* array was detected, so bare items mixed in with real sections were still silently dropped); the synthetic section's `category` is inferred from the majority recognized ID prefix among the rescued findings (falls back to `code_quality` on no clear majority) instead of always hardcoding `code_quality`.
- **`scripts/validate_report.py`** `--producer`: a non-object schema root (e.g. a bare `true`/array) no longer crashes with an `AttributeError` — now exits 2 with a clear schema error.
...
- **`skills/grand-admiral/SKILL.md`**: § Session Protocol gains a mid-turn-interjection triage rule (finish the current atomic unit before context-switching on a non-urgent message); § Terminating Teammates gains proactive orphan-pane-sweep trigger points (after a shutdown wave, after compaction resume); the canonical Monitor watchdog command points `--worktrees` at `${CLAUDIUS_WORKTREE_ROOT:-/data/git-worktrees}`; § Token Economy and § Scaling are replaced with a pointer to the new `spawn-checklist` skill; the GONE event description now distinguishes clock-corroborated `reason=stale-active` from the spoofable pane-title/foreground-command-derived `reason=pane-dead|pid-gone` (any same-UID process can forge the tmux socket binding or foreground command) and requires independent clock corroboration before acting on the latter two; § Multi-Session Hygiene documents that `$CLAUDE_SESSION_ID` can silently mismatch the team-lead session id (`mode="none"` with no warning) and the fix — derive `--team-dir` from a spawn's own returned `agent_id` instead of guessing `--session-id`; § Terminating Teammates' orphan-pane recovery now targets tmux panes by their permanent `pane_id` (`%N`) instead of window-relative index — empirically confirmed on tmux 3.6 that killing multiple panes by index in one pass renumbers survivors after each kill, so a batch computed from one snapshot can hit the wrong pane.
...
- **`skills/codex-crew/SKILL.md`** + `references/sandbox-and-recovery.md`: document never dispatching two Codex jobs back-to-back from the same session cwd without staggering (can silently orphan the earlier job's turn); document that Codex discovery requires a named teammate or explicit `--worktrees`; document the harness-can-kill-a-backgrounded-task failure mode and its `--resume-last` recovery; note the ledger's workspace-local fallback; § Monitoring a Codex Job now leads with direct on-disk job-state polling as the primary method (was framed as a stopgap) and demotes watchdog `CODEX_*` events to best-effort, since a multi-teammate wave's independently-sessioned jobs can make `CODEX_*` silently under-report; the on-disk job-state reference documents `result.rawOutput`/`result.touchedFiles`, corrects the "`pid` is often null" claim (`task-worker`-class jobs carry a real, `/proc`-verifiable pid), and recommends matching jobs to dispatches by `startedAt`/`touchedFiles` rather than `sessionId`.
...
- **`tests/test_minion_monitoring_gone.sh`**: added the zero-tmux-backed-teammate scenario — the exact input shape that produced the PR #54 `OUR_PANE_TYPE` unbound-variable crash — which no prior scenario exercised; the existing suite would have kept passing even with that fix reverted.
