...
class TestSchemaVersionGate:
    def test_merge_fields_silent_on_newer_schema_version(self):
        """The gate is a MINIMUM, not an equality: 3.3.0 carries every 3.2.0
        field, so a consolidated 3.3.0 report must not be nagged about them."""
        report = _report(
            [_section([_finding(1, merge_class="non_blocking")])],
        )
        report["schema_version"] = "3.3.0"
        report["top_findings"] = [
            {
                "id": "CODE-001",
                "severity": 4,
        ...
        assert not any(
            "require schema_version" in w for w in vr.check_consistency(report)
        )

    def test_deferred_to_requires_schema_3_3_0(self):
        report = _report(
            [
                _section(
                    [
        ...
        report["schema_version"] = "3.2.0"
        assert any(
            "deferred_to" in w and "require schema_version=3.3.0" in w
...
class TestUntrackedDeferral:
    """MEDIUM+ deferrals must carry a tracking reference (severity skill:
    out_of_scope_follow_up is 'acceptable to never fix' ONLY when filed)."""

    def _deferred(self, **over: object) -> dict:
        report = _report(
            [_section([_finding(1, merge_class="out_of_scope_follow_up", **over)])]
        )
        report["schema_version"] = "3.3.0"
        return report

    def test_medium_deferral_without_reference_warns(self):
        warnings = vr.check_consistency(
            self._deferred(severity=3, risk=0.5, impact=0.5, scope=0.5)
        )
        assert any(
            "CODE-001" in w and "deferred_to" in w and "file it or reclassify" in w
            for w in warnings
        )

    def test_derived_medium_severity_also_warns(self):
        """Severity is usually derived, not explicit — the floats must count."""
        warnings = vr.check_consistency(self._deferred(risk=0.5, impact=0.5, scope=0.5))
        assert any("file it or reclassify" in w for w in warnings)

    def test_deferral_with_reference_is_silent(self):
        warnings = vr.check_consistency(
            self._deferred(
                severity=4,
                risk=0.75,
                impact=0.75,
                scope=0.75,
                deferred_to="https://example/issues/1",
            )
        )
        assert not any("file it or reclassify" in w for w in warnings)

    def test_blank_reference_counts_as_missing(self):
        warnings = vr.check_consistency(
            self._deferred(
                severity=3, risk=0.5, impact=0.5, scope=0.5, deferred_to="  "
            )
        )
        assert any("file it or reclassify" in w for w in warnings)

    def test_low_deferral_is_silent(self):
        warnings = vr.check_consistency(self._deferred(severity=2))
        assert not any("file it or reclassify" in w for w in warnings)

    def test_other_merge_classes_are_silent(self):
        for merge_class in ("non_blocking", "disputed"):
            report = _report(
                [
                    _section(
                        [
    ...
    def test_warning_does_not_fail_validation(self, tmp_path, monkeypatch, capsys):
        ...
        assert code == 0
        assert "Valid:" in out
        assert "file it or reclassify" in err
