...
# ---------------------------------------------------------------------------
# deferred_to — tracked-deferral reference (schema 3.3.0)
# ---------------------------------------------------------------------------
def _deferred_finding(**over: object) -> dict:
    f = {
        "id": "CODE-001",
        "severity": 3,
...
def test_deferred_ref_url_and_plain():
    linked = grr._deferred_ref({"deferred_to": "https://example/issues/1"})
    assert linked == {
        "text": "https://example/issues/1",
        "url": "https://example/issues/1",
    }
    plain = grr._deferred_ref({"deferred_to": "octo/widgets#42"})
    assert plain == {"text": "octo/widgets#42", "url": None}
    assert grr._deferred_ref({}) is None
    assert grr._deferred_ref({"deferred_to": "  "}) is None
    assert grr._deferred_ref({"deferred_to": "javascript:alert(1)"})["url"] is None


def test_markdown_renders_deferred_to_as_link():
    md = grr.render_markdown(_wrap_section(_deferred_finding()))
    assert (
        "- **Deferred to**: [https://github.com/octo/widgets/issues/42]"
        "(https://github.com/octo/widgets/issues/42)" in md
    )


def test_markdown_renders_non_url_deferred_to_as_plain_text():
    md = grr.render_markdown(
        _wrap_section(_deferred_finding(deferred_to="octo/widgets#42"))
    )
    assert "- **Deferred to**: `octo/widgets#42`" in md
    assert "](octo/widgets#42)" not in md


def test_markdown_omits_deferred_to_when_absent():
    finding = _deferred_finding()
    del finding["deferred_to"]
    assert "Deferred to" not in grr.render_markdown(_wrap_section(finding))


def test_html_and_triage_render_deferred_to_link():
    data = _wrap_section(_deferred_finding())
    for output in (
        grr.render_html(data),
        grr.render_triage(_wrap_section(_deferred_finding())),
    ):
        assert "Deferred to:" in output
        assert 'href="https://github.com/octo/widgets/issues/42"' in output


def test_html_escapes_non_url_deferred_to_without_linking():
    data = _wrap_section(_deferred_finding(deferred_to='TRACK-1"><img src=x>'))
    html = grr.render_html(data)
    assert "<img src=x>" not in html
    assert "&lt;img src=x&gt;" in html


def test_pdf_renders_deferred_to(tmp_path):
    out = tmp_path / "deferred.pdf"
    grr.render_pdf(_wrap_section(_deferred_finding()), out)
    assert out.is_file()
