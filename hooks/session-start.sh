...
if command -v cargo >/dev/null 2>&1; then
  ...
  printf -v RUST_ENV '\n\n## Rust build environment\n- Machine shared target dir: %s — ANY cargo invocation routed through the wrapper AUTO-ISOLATES per checkout (own path-hashed target dir; CLAUDIUS_TARGET_PREFIX changes only its root, while an explicit CARGO_TARGET_DIR via CLAUDIUS_ISOLATE_TARGET=1 wins). The hook forces test/clippy/nextest through it; a routed build isolates too. A raw cargo build NOT via the wrapper uses this shared dir; bare cargo metadata outside the wrapper also reports the shared dir, not the isolated one; undocumented ad-hoc overrides are denied\n- sccache: %s; cargo-nextest: %s\n- Verification ledger: %s\n- Wrapper (absolute path — skill docs only name it relatively): %s — route cargo test/clippy/nextest through it (hook-enforced); build may go through it too for dedup but is not enforced. Identical command + identical tree replays the recorded log instantly (any agent); CLAUDIUS_FORCE=1 forces a real re-run' "$TGT" "$SC" "$NX" "$LEDGER" "$WRAPPER"
fi
