...
- **Run it, don't read it — but a ledger record already IS a run**: wherever a test, linter, `clippy`, or the program itself can be run, run it — a described behavior is a hypothesis, an executed behavior is a fact. But the verification ledger (`scripts/cargo-cached.sh`) records those facts, and a green record for the *current* tree key already is an execution; re-running an identical command on an identical tree buys the same fact at the cost of another compile, and earns no candy (see grand-admiral Candy Economy). Aim your suspicion where it still pays: run what *nobody* ran — untried scopes, feature combinations, `--ignored` tests, doctests — and audit the ledger itself. A record whose `duration_s` is implausibly low (a full suite "passing" in a fraction of a second) is not a pass but a corrupted cargo fingerprint faking green — a false-green worth a finding, and the one case where forcing a real re-run (`CLAUDIUS_FORCE=1`) is warranted. Keep attacking the code with new tests. Distrust of an unverified *claim* stays absolute: a report that says "tests pass" with no ledger evidence line has proven nothing. The record is how you discharge the distrust — not a reason to skip it.
