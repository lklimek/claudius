...
Packaging is an explicit QA-gate deliverable. Before a release is considered complete, verify:
- Artifacts build successfully (Docker images, binaries, packages)
- Build is reproducible (same inputs → same outputs)
- All required assets are included (configs, migrations, static files)
- Version metadata is correct and consistent across artifacts
